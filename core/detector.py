...
class ThreatDetector:
    ...
    def _alert(self, alert_type, src_ip, dst_ip, details, severity="MEDIUM"):
        ...
        icons = {'LOW': '[!]', 'MEDIUM': '[!!]', 'HIGH': '[!!!]', 'CRITICAL': '[CRITICAL]'}
        icon = icons.get(severity, '[!]')
        print(f"{icon} ALERT #{self.alert_count} | {alert_type} | {src_ip} -> {dst_ip} | {details}")
    ...
    def _check_syn_flood(self, packet, src_ip, dst_ip):
        if packet['TCP'].flags == 'S':
            ...
            self.syn_tracker[src_ip] = [t for t in self.syn_tracker[src_ip] if now - t < self.config.get('syn_window', 10)]
            if len(self.syn_tracker[src_ip]) >= self.config.get('syn_threshold', 100):
                self._alert("SYN FLOOD", src_ip, dst_ip, f"{len(self.syn_tracker[src_ip])} SYN packets in 10 seconds", "CRITICAL")
                self.syn_tracker[src_ip] = []

    def _check_port_scan(self, packet, src_ip, dst_ip):
        if packet['TCP'].flags in ['S', 'F', 'N']:
            dst_port = packet['TCP'].dport
            self.port_scan_tracker[src_ip].add(dst_port)
            if len(self.port_scan_tracker[src_ip]) >= self.config.get('port_scan_threshold', 20):
                self._alert("PORT SCAN", src_ip, dst_ip, f"{len(self.port_scan_tracker[src_ip])} unique ports scanned", "HIGH")
                self.port_scan_tracker[src_ip] = set()

    def _check_brute_force(self, packet, src_ip, dst_ip):
        brute_ports = self.config.get('brute_force_ports', [22, 21, 3389, 5900, 23])
        dst_port = packet['TCP'].dport
        if dst_port in brute_ports and packet['TCP'].flags == 'S':
            now = time.time()
            self.brute_force_tracker[src_ip].append(now)
            self.brute_force_tracker[src_ip] = [t for t in self.brute_force_tracker[src_ip] if now - t < self.config.get('brute_force_window', 60)]
            if len(self.brute_force_tracker[src_ip]) >= self.config.get('brute_force_threshold', 10):
                service = {22: 'SSH', 21: 'FTP', 3389: 'RDP', 5900: 'VNC', 23: 'Telnet'}.get(dst_port, str(dst_port))
                self._alert("BRUTE FORCE", src_ip, dst_ip, f"{len(self.brute_force_tracker[src_ip])} attempts on {service}", "HIGH")
                self.brute_force_tracker[src_ip] = []

    def _check_icmp_flood(self, packet, src_ip, dst_ip):
        now = time.time()
        self.icmp_tracker[src_ip].append(now)
        self.icmp_tracker[src_ip] = [t for t in self.icmp_tracker[src_ip] if now - t < self.config.get('icmp_window', 10)]
        if len(self.icmp_tracker[src_ip]) >= self.config.get('icmp_threshold', 50):
            self._alert("ICMP FLOOD", src_ip, dst_ip, f"{len(self.icmp_tracker[src_ip])} ICMP packets in 10 seconds", "HIGH")
        ...
        if packet['ARP'].op == 2:
            ...
            if src_ip in self.arp_table:
                if self.arp_table[src_ip] != src_mac:
                    self._alert("ARP SPOOFING", src_ip, "broadcast", f"MAC changed: {self.arp_table[src_ip]} -> {src_mac}", "CRITICAL")
            else:
    ...
    def _check_dns_anomaly(self, packet, src_ip):
        if packet.haslayer('DNS') and packet['DNS'].qr == 0:
            try:
                ...
                self.dns_tracker[src_ip] = [t for t in self.dns_tracker[src_ip] if now - t < self.config.get('dns_window', 10)]
                if len(self.dns_tracker[src_ip]) >= self.config.get('dns_threshold', 50):
                    self._alert("DNS FLOOD", src_ip, "DNS server", f"{len(self.dns_tracker[src_ip])} DNS queries in 10 seconds", "MEDIUM")
